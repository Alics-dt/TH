...
class InvertedResidual(nn.Module):
    def __init__(self, input_c: int, output_c: int, stride: int, k_size):
        ...
        if self.stride == 2:
            ...
            self.branch2_end = nn.Sequential(
                self.depthwise_conv(features_channel, features_channel, kernel_s=3, stride=2, padding=1),
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        else:          # stride=2时，输出需经过3*3dw和1*1的pw卷积
...
model = ShuffleNetV2(stages_repeats=[3, 7, 3],
...
更改日志：
stages_out_channels=[24, 176, 352, 704, 1024]
1：stage2中的第一个block无branch2_end模块，直接channel-shuffle。Flops大小由61M变为91M.  params：3.16M变为3.02M
2：改激活函数relu为Hswish。
